...
def compute_score_batch(
    ...
    **kwargs,
    ...
    if false_positive_rate <= 0 and false_negative_rate <= 0:
        print(f"[Batch Scorer]  No noise requested, using original scoring method")
    ...
    if train_indices:
        ...
        if method == "sandbox":
            ...
            print(f"[Batch Scorer]  Unknown method: {method}, defaulting to binary sandbox with noise")
            train_results = _sandbox_rewards_with_noise(
                train_solutions, 
                train_verification_infos, 
                false_positive_rate=false_positive_rate,
